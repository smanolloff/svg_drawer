module SvgDrawer
  class Polyline < Base
    ...
    def min_x
      @min_x ||= @points.each_slice(2).min_by(&:first).first - linecap_size
    end

    def max_x
      @max_x ||= @points.each_slice(2).max_by(&:first).first + linecap_size
    end

    def min_y
      @min_y ||= @points.each_slice(2).min_by(&:last).last - linecap_size
    end

    def max_y
      @max_y ||= @points.each_slice(2).max_by(&:last).last + linecap_size
    end
    ...
    def _render(parent)
      ...
      style[:'stroke-dasharray'] = "#{size}, #{dotspace}"

      RasemWrapper.group(parent, class: 'polyline') do |polyline_group|
    ...
    def linecap_size
      case param(:linecap)
      when 'round' then param(:size).to_d / 2
      when 'square' then param(:size).to_d / 2
      when 'butt' then 0
      else raise "Bad linecap: #{param(:linecap)}. Valid are: [round, square, butt]"
      end
    end
  end
end
